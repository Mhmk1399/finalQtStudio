...
interface DynamicBarProps {
  dataType: 'customers' | 'contracts' | 'projects' | 'tasks' | 'teams' | 'users' | 'services' | 'serviceRequests';
  data?: ChartDataItem[];
  endpoint?: string;
...
const COLOR_PALETTES = {
  primary: ['#8B5CF6', '#EC4899', '#3B82F6', '#10B981', '#F59E0B', '#EF4444', '#6366F1', '#14B8A6'],
  secondary: ['#A78BFA', '#F472B6', '#60A5FA', '#34D399', '#FBBF24', '#F87171', '#818CF8', '#2DD4BF'],
...
const DynamicBar: React.FC<DynamicBarProps> = ({
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const result = await response.json() as { success: boolean; data?: ChartDataItem[]; error?: string };
  ...
  const processedData = useMemo(() => {
    ...
    const grouped = data.reduce<Record<string, ProcessedDataItem>>((acc, item:{
      ...
      if (config.valueKey !== 'count' && typeof item[config.valueKey] === 'number') {
      ...
      return (
        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
          ...
          <p className="text-blue-600">
            {config.valueKey === 'count' ? 'تعداد' : 'مقدار'}: {payload[0].value.toLocaleString('fa-IR')}
          </p>
        </div>
      );
    }
        ...
        return (
          <PieChart>
            <Pie
              ...
              cy="50%"
              labelLine={false}
              label={({ name, percent = 0 }: PieDataEntry) => `${name} ${(percent * 100).toFixed(0)}%`}
              outerRadius={80}
              fill="#8884d8"
              dataKey="value"
            >
    ...
    return (
      <div className="w-full bg-white rounded-lg shadow-lg p-6">
        <div className="text-center text-red-600">
          <p className="text-lg font-semibold mb-2">خطا در بارگذاری داده‌ها</p>
    ...
    return (
      <div className="w-full bg-white rounded-lg shadow-lg p-6">
        <div className="text-center text-gray-500">
          <p className="text-lg font-semibold mb-2">داده‌ای موجود نیست</p>
          <p className="text-sm">هنوز هیچ داده‌ای برای نمایش وجود ندارد</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">
            مجموع: {processedData?.reduce((sum, item) => sum + item.value, 0).toLocaleString('fa-IR')}
          </p>
        </div>
        <div className="flex items-center space-x-2 space-x-reverse">
          ...
          <div className="flex items-center space-x-1 space-x-reverse">
            <div className={`w-3 h-3 rounded-full ${
              config.type === 'bar' ? 'bg-blue-500' :
              config.type === 'pie' ? 'bg-green-500' :
              config.type === 'line' ? 'bg-purple-500' :
              'bg-orange-500'
            }`}></div>
            <span className="text-xs text-gray-500 capitalize">{config.type}</span>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="text-center">
            <p className="text-2xl font-bold text-green-600">
              {Math.max(...processedData.map(item => item.value)).toLocaleString('fa-IR')}
            </p>
            <p className="text-xs text-gray-500">بیشترین</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-orange-600">
              {Math.min(...processedData.map(item => item.value)).toLocaleString('fa-IR')}
            </p>
            <p className="text-xs text-gray-500">کمترین</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-purple-600">
              {Math.round(processedData.reduce((sum, item) => sum + item.value, 0) / processedData.length).toLocaleString('fa-IR')}
            </p>
...
export const CustomerChart: React.FC<Omit<DynamicBarProps, 'dataType'>> = (props) => (
  <DynamicBar {...props} dataType="customers" />
);

export const ContractChart: React.FC<Omit<DynamicBarProps, 'dataType'>> = (props) => (
  <DynamicBar {...props} dataType="contracts" />
);

export const ProjectChart: React.FC<Omit<DynamicBarProps, 'dataType'>> = (props) => (
  <DynamicBar {...props} dataType="projects" />
);

export const TaskChart: React.FC<Omit<DynamicBarProps, 'dataType'>> = (props) => (
  <DynamicBar {...props} dataType="tasks" />
);

export const TeamChart: React.FC<Omit<DynamicBarProps, 'dataType'>> = (props) => (
  <DynamicBar {...props} dataType="teams" />
);

export const UserChart: React.FC<Omit<DynamicBarProps, 'dataType'>> = (props) => (
  <DynamicBar {...props} dataType="users" />
);

export const ServiceChart: React.FC<Omit<DynamicBarProps, 'dataType'>> = (props) => (
  <DynamicBar {...props} dataType="services" />
);

export const ServiceRequestChart: React.FC<Omit<DynamicBarProps, 'dataType'>> = (props) => (
  <DynamicBar {...props} dataType="serviceRequests" />
);
...
export const ChartDashboard: React.FC<DashboardProps> = ({ charts, refreshInterval }) => {
  return (
...
export const useChartData = (dataType: DynamicBarProps['dataType'], endpoint?: string) => {
  const [data, setData] = useState<ChartDataItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
